...
class FeatureConfig:
    ...
    中文：控制构建 X1/X2/X3 特征集合所需的窗口长度等参数。
    """
...
def rolling_group_apply(
    ...
    中文：对每个 PERMNO 分组执行滚动计算，并将结果对齐回原始索引。
    """
...
def build_feature_set_x1(panel: pd.DataFrame, config: FeatureConfig) -> pd.DataFrame:
    ...
    中文：依据文档公式使用 r/to 构造 X1 特征（收益/换手的累计指标）。
    """
    df = panel.copy()
...
def build_feature_set_x2(panel: pd.DataFrame, config: FeatureConfig) -> pd.DataFrame:
    ...
    中文：依据文档公式构造 X2 特征（排除最近一期的归一化累计收益）。
    """
    df = panel.copy()
    lookbacks = [3, 6, 9, 12]
    group = df.groupby("PERMNO")
...
def build_feature_set_x3(panel: pd.DataFrame, config: FeatureConfig) -> pd.DataFrame:
    ...
    中文：依据文档公式构造 X3 特征（过去 12 个月的标准化月度收益）。
    """
    df = panel.copy()
...
def assemble_feature_matrix(
    ...
    中文：将带特征的数据框打包为模型训练所需的 numpy 矩阵和标记。
    """
    feature_cols = [
        col
