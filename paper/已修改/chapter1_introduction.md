...
**RQ2: Within the hybrid-loss design space, which combination of directional and robust components produces the best multi-seed Sharpe and stability?** This question is answered in two stages. First, a single-seed design-space mapping (Phase 2, Chapter 5 §5.3) identifies the most promising hybrid families. Second, the M2-robust γ family and the IMADL-m2 α family are evaluated across three seeds (Phase 3, Chapter 5 §5.4–§5.5) to determine which loss-family region produces the best mean Sharpe with tolerable coefficient of variation. The additive A-series and multiplicative M1–M4 are evaluated only at seed 42 and are not carried into multi-seed evidence.
