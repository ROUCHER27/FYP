...
- Feature set X1: the X1 feature set contributes 10 engineered columns (cumulative returns and cumulative turnover at 1-, 3-, 6-, 9-, and 12-month horizons); the model input is 15 columns comprising these 10 X1 columns together with the 5 base panel columns (`RET`, `VOL`, `SHROUT`, `r`, `to`) that `assemble_feature_matrix` retains (see Chapter 4 Table 4.3).
- Model: `MLPConfig(input_dim=15, hidden_dims=[64, 32, 16], activation='relu', dropout=0.2)`.
- Training: `max_epochs=20`, `batch_size=1024`, mini-batch training on a single static window; no early stopping.
...
The same-window baseline and Phase 2 hybrid-sweep evidence was run from `main` at commit `6c0fbde` and is stored under `doc/final_report_all_24m_evidence/`. Every summary JSON has a matching `*_verification.json` confirming the 24-row window, and every runner command is recorded under `*_command.txt`. The multi-seed evidence (Phase 3 γ refinement and the integrated summary) lives under `doc/phase2-fix/`; source paths are listed in each table caption.
...
This phase extends the two hybrid multiplicative variants of §5.2 into a parameterised family. The additive variants A1–A5 (`hybrid_add_a{1..5}`) vary the weighting of a directional-accuracy term added to an MSE backbone, while the multiplicative variants M1–M4 (`hybrid_mul_m{1..4}`) vary the weighting of a robust monotone component multiplied into a base prediction error. All nine runs use the same static window, feature set, and portfolio construction as Table 5.1; only the loss specification changes.
...
**Figure 5.2 — Phase 1.5 hybrid variants.**

![Figure 5.2: Phase 1.5 additive and multiplicative variants Sharpe](figures/fig5_2_phase15_variants.png)
