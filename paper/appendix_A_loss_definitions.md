...
The robustness schedule $H^{\gamma}(e)$ is a smooth monotone function that approaches $\tfrac12 e^2$ for small $|e|$ and saturates above a threshold controlled by $\gamma$. Small $\gamma$ flattens the loss surface and prioritises the directional component; large $\gamma$ recovers a nearly unbounded quadratic behaviour. The γ refinement of Chapter 5 §5.4 scans $\gamma \in \{0.03, 0.05, 0.07, 0.10, 0.15\}$. The exact closed form used in the Phase 2.2 runner lives on the `phase2.2-fix` branch together with the runner scripts; the empirically tuned optimum is $\gamma = 0.07$ (see Chapter 5 §5.4 and Table 5.3).
