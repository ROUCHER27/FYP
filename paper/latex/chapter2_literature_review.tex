...
\begin{enumerate}
 ...
 \item \textbf{Static out-of-sample protocol with no in-window tuning.} The model architecture is fixed within each evidence phase (MLP[64,32,16] throughout; Phase~1/2 use ReLU+dropout~0.2, Phase~3 uses tanh+dropout~0.0), chosen via grid search on pre-test data and then frozen. Hyperparameters such as batch size and epoch count are held fixed across losses within each phase. No parameter is tuned on the 1995--1996 test window.
\end{enumerate}
