...
This chapter reports the empirical performance of every loss function that enters the final recommendation. All numbers quoted here are traceable to a single source-of-truth evidence map; captions record the evaluation window and seed set, and Appendix~B records the underlying artifact paths and reproduction commands needed to audit each claim.
...
All experiments share the same evaluation window (train \texttt{1990-01..1994-12}, test \texttt{1995-01..1996-12}, 24 months out-of-sample), the X1 feature input from Chapter~4, and the top/bottom 10\% signal-tilted long-short portfolio with 5\% per-name cap. Sharpe is annualised as $\sqrt{12} \cdot \bar r / \sigma_r$. Single-seed tables (\S5.2--\S5.3) use seed 42; multi-seed tables (\S5.4--\S5.6) use three seeds per row.

The model and training configuration differs between evidence phases. Phase~1 and Phase~2 single-seed evidence (Tables~5.1--5.2) was produced on \texttt{main} at commit \texttt{6c0fbde} with MLP[64,32,16]+ReLU+dropout~0.2, batch size 1024, 20 epochs. Phase~3 multi-seed evidence (Tables~5.3--5.5) was produced on the \texttt{phase2.2-fix} branch with MLP[64,32,16]+tanh+dropout~0.0, batch size 1024, 20 epochs. Within each table the model configuration is identical across rows; only the loss function varies. Cross-phase comparisons therefore differ in activation and dropout in addition to the loss, and are discussed with appropriate caveats in \S5.7. Source paths are listed in each table caption; full reproduction commands are in Appendix~B.
...
\begin{table}[H]
\centering
\caption{Baseline loss comparison. Static train \texttt{1990-01..1994-12}, test \texttt{1995-01..1996-12}, seed 42, MLP[64,32,16]+ReLU+dropout 0.2, X1, batch 1024, 20 epochs, top/bottom 10\% with 5\% per-name cap. Source: Appendix~B, Section~B.4.}
...
\begin{table}[H]
\centering
\caption{Phase 2 additive (A1--A5) and multiplicative (M1--M4) variants. Static train \texttt{1990-01..1994-12}, test \texttt{1995-01..1996-12}, seed 42, same model / feature / portfolio configuration as Table~\ref{tab:baseline-loss-comparison}. Source: Appendix~B, Section~B.4.}
...
Phase~2 introduces an explicit prediction-variance regularisation parameter $\gamma$ into the multiplicative hybrid M2 variant, producing the family of \texttt{m2\_robust\_gamma\{03, 05, 07, 10, 15\}} losses (corresponding to $\gamma = 0.3, 0.5, 0.7, 1.0, 1.5$ in the implementation; see Chapter~3 \S\ref{subsec:m2-robust-gamma} for the naming convention). Each variant is run across three random seeds using the same 24-month static window and the 5\% per-name portfolio cap (\texttt{cap05}). The purpose is to decouple the seed-42 ranking of \S5.3 from a claim about the expected performance of the loss under seed perturbation.
...
\begin{table}[H]
\centering
\caption{Phase 3a $\gamma$ refinement, multi-seed (3 seeds per row). Static train \texttt{1990-01..1994-12}, test \texttt{1995-01..1996-12}, \texttt{cap05} portfolio cap. Each row aggregates three seeds. Source: Appendix~B, Section~B.4.}
...
Third, the $\gamma$ schedule exhibits a non-monotone relationship with Sharpe stability. \texttt{gamma03} has a negative seed and high CV ($1.0570$), indicating that $\gamma = 0.3$ provides insufficient variance regularisation for seed-robust performance. \texttt{gamma15} -- past the stability peak -- also degrades sharply in CV ($0.4562$) even though its mean Sharpe remains respectable ($0.8163$). The stability peak near $\gamma = 0.7$ (loss id \texttt{gamma07}) is thus an internal optimum of the variance-penalty strength rather than the endpoint of a monotone trend. This is consistent with the design intent of the variance regulariser: too little penalty under-regularises the prediction spread; too much penalty over-compresses the signal and re-injects seed-level noise into the learned weights.
...
Source: integrated grouped summary CSV (Appendix~B, Section~B.3 records the exact branch path). All rows are 3-seed averages at \texttt{cap05} with the same 24-month window as Table~\ref{tab:gamma-refinement}.
...
Figure~\ref{fig:sharpe-cv-frontier} places all multi-seed hybrid variants on a single Sharpe-stability plane, making the frontier structure visible. The preferred region -- high Sharpe, low CV ($\leq 0.35$) -- contains \texttt{gamma07} at the highest Sharpe among the low-CV points; \texttt{gamma05} and \texttt{alpha04} also satisfy the CV threshold but at substantially lower mean Sharpe. \texttt{gamma10} lies outside the preferred region on the CV axis despite its higher mean Sharpe. \texttt{alpha06} lies inside the preferred region on the CV axis but below \texttt{gamma07} on the Sharpe axis. No other variant from the $\beta$ or adaptive-$\lambda$ families reaches the preferred region. The frontier view confirms that \texttt{gamma07} achieves the best joint Sharpe-stability trade-off across the full hybrid design space explored in this study.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig5_6_sharpe_cv_frontier.png}
\caption[Sharpe-stability frontier across multi-seed hybrid families]{Sharpe-stability frontier across multi-seed hybrid families. Each point is a 3-seed grouped summary. x-axis: cross-seed CV (lower = more stable); y-axis: mean annualised Sharpe; marker size proportional to mean cumulative return. Blue shading marks the preferred region (CV $\leq 0.35$). Red star: \texttt{m2\_robust\_gamma07} (recommended); orange triangle: \texttt{m2\_robust\_gamma10}; green diamond: \texttt{imadl\_m2\_alpha06} (stable fallback). Rows with CV $> 8$ (IMADL-GMADL $\beta$05 and $\beta$07) are excluded and noted in the figure. Evaluation window \texttt{1995-01..1996-12}, \texttt{cap05}, 3 seeds per row.}
...
\begin{table}[H]
\centering
\caption{Loss-component normalisation probe. Static train \texttt{1990-01..1994-12}, test \texttt{1995-01..1996-12}, \texttt{cap05}, 3 seeds per row. Scale ratio is defined as the mean absolute directional-component loss divided by the mean absolute MSE-component loss, averaged over all training batches across three seeds (diagnostics-estimated; per-component logger not yet implemented). Source: Appendix~B, Section~B.4.}
