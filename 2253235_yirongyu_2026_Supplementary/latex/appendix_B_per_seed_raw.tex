...
(The per-seed IDs are anonymised in the summary JSON. The project notes record that the normalisation probe uses seeds \texttt{\{42, 123, 456\}}, but the mapping between the numeric position and those seed IDs is not preserved in the summary file; the mean and per-seed spread remain the relevant quantities.)
...
\item \texttt{best\_hyperparameters.txt} is unchanged from commit \texttt{6c0fbde} (MLP[64, 32, 16], ReLU, dropout 0.2, input\_dim 15). This applies to Phase~1 and Phase~2 single-seed evidence; Phase~3 multi-seed evidence on \texttt{phase2.2-fix} uses tanh activation and dropout 0.0.
...
1 & VOL & monthly trading volume (shares) for month $t$ \\
2 & RET & raw monthly total return for month $t$ \\
